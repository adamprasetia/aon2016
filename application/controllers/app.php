...
class App extends MY_Controller {
	...
	function export(){
		...
		$active_sheet->getStyle("A1:AS1")->getFont()->setBold(true);
		...
		$active_sheet->setCellValue('AH1', 'OE1');
		$active_sheet->setCellValue('AI1', 'OE2');
		$active_sheet->setCellValue('AJ1', 'D1');
		$active_sheet->setCellValue('AK1', 'D2');
		$active_sheet->setCellValue('AL1', 'D3');
		$active_sheet->setCellValue('AM1', 'D4');
		$active_sheet->setCellValue('AN1', 'D5');
		$active_sheet->setCellValue('AO1', 'D6');
		$active_sheet->setCellValue('AP1', 'AUDIT');
		$active_sheet->setCellValue('AQ1', 'SALAH');
		$active_sheet->setCellValue('AR1', 'User Entry');
		$active_sheet->setCellValue('AS1', 'Date Entry');
		$active_sheet->setCellValue('AT1', 'Country');
		...
		foreach($result as $r){
			...
			$active_sheet->setCellValue('AH'.$i, strtolower($r->com_1));
			$active_sheet->setCellValue('AI'.$i, strtolower($r->com_2));
			$active_sheet->setCellValue('AJ'.$i, ($r->dem_1<>0?$r->dem_1:""));
			$active_sheet->setCellValue('AK'.$i, ($r->dem_2<>0?$r->dem_2:""));
			$active_sheet->setCellValue('AL'.$i, ($r->dem_3<>0?$r->dem_3:""));
			$active_sheet->setCellValue('AM'.$i, ($r->dem_4<>0?$r->dem_4:""));
			$active_sheet->setCellValue('AN'.$i, ($r->dem_5<>0?$r->dem_5:""));
			$active_sheet->setCellValue('AO'.$i, ($r->dem_6<>0?$r->dem_6:""));
			$active_sheet->setCellValue('AP'.$i, $r->audit);
			$active_sheet->setCellValue('AQ'.$i, $r->salah);
			$active_sheet->setCellValue('AR'.$i, $r->fullname);
			$active_sheet->setCellValue('AS'.$i, PHPExcel_Shared_Date::PHPToExcel(date_to_excel($r->date_create)));
			$active_sheet->getStyle('AS'.$i)->getNumberFormat()->setFormatCode('dd/mm/yyyy');		   
			$active_sheet->setCellValue('AT'.$i, $r->country);
			$i++;
		}
